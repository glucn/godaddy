...
func (l *gkeLogger) request(ctx context.Context, rl *requestData, stream Stream) {
...
func (l *gkeLogger) StackTrace(ctx context.Context, f string, a ...interface{}) {
...
func (l *gkeLogger) logRequest(ctx context.Context, rd *requestData, stream Stream) {
	sev := logging.Info
	...
	if rd.HTTPRequest.Request.URL != nil {
		tags = append(tags, fmt.Sprintf("path:%s", rd.HTTPRequest.Request.URL.Path))
	}
...
func newGkeLogger(config *config, client *logging.Client) (*gkeLogger, error) {
	...
	logger := gkeLogger{
		...
		commonLoggers: map[Stream]*logging.Logger{
			Request:   client.Logger(string(Request), logging.CommonResource(mr)),
			Pubsub:    client.Logger(string(Pubsub), logging.CommonResource(mr)),
			Taskqueue: client.Logger(string(Taskqueue), logging.CommonResource(mr)),
		},
	}
	return &logger, nil
}
