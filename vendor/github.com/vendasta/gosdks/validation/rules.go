...
// Validate returns an error if predicate is true and one or more rules failed to validate.
func (v *optionalValidator) Validate() error {
	if v.predicate {
...
// Optional validates the given rules if predicate is true, otherwise returns nil
func Optional(predicate bool, rules ...Rule) Rule {
	rule := NewValidator().Rule(rules...)
...
func Int64Not0(data int64, errorType util.ErrorType, message string) Rule {
...
func Float64Not0(data float64, errorType util.ErrorType, message string) Rule {
...
func TimeNot0(data time.Time, errorType util.ErrorType, message string) Rule {
...
// used, otherwise >
func TimeAfter(laterTime time.Time, earlierTime time.Time, inclusive bool, errorType util.ErrorType, message string) Rule {
	return &timeAfter{
		laterTime:   laterTime,
...
func MaxInt(data int, max int, errorType util.ErrorType, message string) Rule {
...
func IntGreaterThan(data int64, bound int64, errorType util.ErrorType, message string) Rule {
...
func IntLessThan(data int64, bound int64, errorType util.ErrorType, message string) Rule {
...
func IntBetween(data int, min int, max int, errorType util.ErrorType, message string) Rule {
...
func FloatBetween(data float64, min float64, max float64, errorType util.ErrorType, message string) Rule {
...
//
//   ValueNotNil(&myStruct{}, util.InvalidArgument, "error message")
func ValueNotNil(data interface{}, errorType util.ErrorType, message string) Rule {
	return &valueNotNil{data: data, errorType: errorType, message: message}
}

func (r *valueNotNil) Validate() error {
	if r.data == nil || (reflect.ValueOf(r.data).Kind() == reflect.Ptr && reflect.ValueOf(r.data).IsNil()) {
		return util.Error(util.InvalidArgument, r.message)
	}
	return nil
}
...
func BoolTrue(value bool, errorType util.ErrorType, message string) Rule {
...
func BoolNotFalse(value bool, errorType util.ErrorType, message string) Rule {
...
func BoolFalse(value bool, errorType util.ErrorType, message string) Rule {
